...
class HierachicalEncoder(nn.Module):
    ...
    def forward_all(self, test=False):
        ...
        k = final_feature 
        final_feature_enhanced, _ = self.light_gcn(k, self.iui_edge_index, return_attention_weights=True)
        ...
        final_feature_modal_enhanced, _ = self.ii_modal_sim_gat(
            k,
            self.mm_adj.coalesce(),
            return_attention_weights=True
        )
...
class CLHE(nn.Module):
    ...
    def forward(self, batch):
        ...
        combine_loss = {
            'loss': loss + item_loss + bundle_loss +self.alpha_balance_loss*balance_loss,
            # 'loss': loss,
            'item_loss': loss,
            'bundle_loss': loss
        }
