...
class CLHE(nn.Module):
    ...
    def forward(self, batch):
        ...
        feat_bundle_view, bundle_gat_emb, bundle_modal_emb, bundle_cross_emb, _, bundle_f, b_ii = self.encoder(seq_full)  # [bs, n_token, d]

        # bundle feature construction >>>
        bundle_feature = self.bundle_encode(feat_bundle_view, mask=mask)

        feat_retrival_view, item_gat_emb, item_modal_emb, cross_modal_item_emb, _, item_f, i_ii = self.decoder(batch, all=True)

        # option 1 
        bundle_feature = bundle_feature + bundle_gat_emb[idx] + bundle_modal_emb[idx] + b_ii[idx]
        feat_retrival_view = feat_retrival_view + item_gat_emb + item_modal_emb + i_ii
    ...
    def evaluate(self, _, batch):
        ...
        feat_bundle_view, bundle_gat_emb, bundle_modal_emb, bundle_cross_emb, _, bundle_f, b_ii = self.encoder(seq_x, test=True)

        bundle_feature = self.bundle_encode(feat_bundle_view, mask=mask)

        feat_retrival_view, item_gat_emb, item_modal_emb, cross_modal_item_emb, _, item_f, i_ii = self.decoder(
