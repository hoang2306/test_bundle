...
class BundleTrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        full = torch.from_numpy(
            self.b_i_graph[self.bundles_map[index]].toarray()
        ).squeeze()
...
class Datasets():
    def __init__(self, conf):
        ...
        self.bundle_val_data = BundleTestDataset(
            ...
            b_i_graph_val_gt, self.num_bundles, self.num_items
        )
        self.bundle_test_data = BundleTestDataset(
            conf, b_i_pairs_test_i, b_i_graph_test_i, b_i_pairs_test_gt, 
            b_i_graph_test_gt,self.num_bundles, self.num_items
        )
    ...
    def get_bi(self, task):
        ...
        b_i_pairs_i = list2pairs(
            os.path.join(self.path, self.name, f'bi_{task}_input.txt')
        )
        b_i_pairs_gt = list2pairs(
            os.path.join(self.path, self.name, f'bi_{task}_gt.txt')
        )
